...
// Generated 28-Jul-2021 14:48:40
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"Controller_MPC\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(Controller_MPC_M, (NULL));
        Controller_MPC_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(Controller_MPC_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(Controller_MPC_M)
                == (NULL)) || rtmGetStopRequested(Controller_MPC_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(Controller_MPC_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
